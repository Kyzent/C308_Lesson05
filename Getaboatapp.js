...
const Getaboatapp = () => {
  ...
  for (let i = 0; i < PRODUCTS_DATA.length; i++) {
    ...
    boatsArr.push(<Text key={boats.name}>{boats.name}</Text>);
    boatsArr.push(<Text key={boats.description}>{boats.description}</Text>);
    boatsArr.push(<Image source={boats.picture} />);
